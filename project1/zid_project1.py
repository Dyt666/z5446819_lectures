...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is None:
        ...
        for i in tickers_lst:
            if i in tic_exchange_dic.keys():
                continue
            else:
                raise Exception(f"{i} is not a key in tic_exchange_dic")
    ...
    if col_lst is None:
        ...
        for i in col_lst:
            if i in COLUMNS:
                ...
                raise Exception(f"{i} is not found in `COLUMNS`")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for tic in tickers_lst:
        ticker_dict = {}
        ticker_dict['exchange'] = tic_exchange_dic[tic]
        ...
        for line in lines:
            content = {}
            for col in col_lst:
                content[col] = line_to_dict(line)[col]
            data.append(content)

        ticker_dict['data'] = data
        dict[tic] = ticker_dict
